...
export default function WalletDepositPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? 'Dépôt échoué');
        return;
      }

      setSuccess(`Dépôt initié (réf: ${data.transaction_id?.slice(0, 8).toUpperCase()}). Confirmez sur votre téléphone.`);
    ...
    <main className="flex flex-col items-center px-4 py-8 max-w-md mx-auto w-full gap-6">
      <div className="flex items-center gap-3 w-full">
        ...
        <h1 className="text-xl font-bold flex items-center gap-2">
          <ArrowDownCircle className="text-[#00A651]" size={22} />
          Dépôt Mobile Money
        </h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="w-full bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-6 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Numéro Mobile Money</label>
          <input
            type="tel"
            value={phoneMM}
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            className="border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-[#00A651]"
          />
          <p className="text-xs text-gray-400">Minimum 100 CDF · Frais 3%</p>
        </div>
