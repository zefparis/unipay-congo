...
export default function DepositMMForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 px-4 py-5">
      ...
      <div className="flex flex-col gap-1.5">
        <label className="text-sm font-heading font-semibold text-ink/70">Montant à déposer (CDF)</label>
        ...
        {num >= minAmt && (
          <div className="bg-ink/5 rounded-lg px-3 py-2 flex justify-between text-xs text-ink/60">
            ...
            <span>Reçu : <strong className="text-signal">{netAmt.toLocaleString('fr-FR')} CDF</strong></span>
          </div>
        )}
        <p className="text-xs text-ink/40">Minimum {minAmt} CDF</p>
      </div>
