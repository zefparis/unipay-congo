    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <form onSubmit={handleFilter} className="flex flex-wrap gap-3">
        <input
          ...
          className="flex-1 min-w-48 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-signal/30 font-mono"
        />
        <select value={filterDir} onChange={(e) => setFilterDir(e.target.value)} className="px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-900 dark:text-white">
        ...
        <select value={filterOp} onChange={(e) => setFilterOp(e.target.value)} className="px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-900 dark:text-white">
        ...
        <input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 text-sm text-gray-900 dark:text-white" />
                ...
                {rows.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{tx.operator}</td>
                      <td className="px-4 py-3 font-mono text-gray-700 dark:text-gray-300">{tx.phone}</td>
                      <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{fmt(tx.amount)}</td>
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{fmt(tx.fee)}</td>
                      <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{fmt(tx.net_amount)}</td>
                      <td className="px-4 py-3 whitespace-nowrap">
                      ...
                      <td className="px-4 py-3 text-gray-500 dark:text-gray-400 font-mono text-xs">{tx.reference ?? '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
