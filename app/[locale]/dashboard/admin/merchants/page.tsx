    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Métriques globales des marchands B2B UniPay Congo</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
      ...
      <div className="bg-white dark:bg-gray-900/60 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
        <h2 className="text-sm font-semibold text-gray-900 dark:text-white mb-4">Volume transactions (30 derniers jours)</h2>
        {volumeEntries.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {volumeEntries.map(([currency, volume]) => (
              <div key={currency} className="border border-gray-100 dark:border-gray-800 rounded-xl p-4">
                <div className="text-xl font-heading font-bold text-gray-900 dark:text-white">
                  {fmt(volume)}
                  <span className="text-sm font-normal text-gray-500 dark:text-gray-400 ml-1.5">{currency}</span>
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Volume net (success)</div>
              </div>
            ))}
          </div>
        )}
